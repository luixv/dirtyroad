            ...
            <div class="bpbm-search-results-section">
                ...
                <div class="threads-list">
                    <?php foreach ( $threads as $thread ) {
                        $thread->subject = ltrim($thread->subject, 'Re: ');
        ...
        public function delete_message()
        {
            ...
            foreach( $messages_ids as $message_id ){
                ...
                if( $message->sender_id === $user_id ){
                    ...
                    $sql = $wpdb->prepare("DELETE FROM " . bpbm_get_table('messages') . " WHERE id = %d", $message->id);
                    $wpdb->query( $sql );
                    $sql = $wpdb->prepare("DELETE FROM " . bpbm_get_table('meta') . " WHERE message_id = %d", $message->id);
                    $wpdb->query( $sql );

                    $recipients = BP_Messages_Thread::get_recipients_for_thread( $message->thread_id );
                    do_action('bp_better_messages_message_deleted', $message->id, array_keys( $recipients ) );
                }
            }
