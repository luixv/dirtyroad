...
class Youzify_Wall {
    ...
    public function __construct() {
    	...
    	// Add Wall Sidebar.
		add_action( 'youzify_global_wall_sidebar', array( $this, 'get_wall_sidebar' ) );
    ...
    function add_comment_attachments( $content ) {
		...
		if ( ! empty( $attachments ) ) {
			...
			switch ( $type ) {
				...
				echo apply_filters( 'youzify_get_wall_post_video', '<video width="100%" controls preload="metadata" type="video/quicktime"><source src="' . wp_get_attachment_url( $attachment_id ) . '#t=0.001" type="video/mp4">' . __( 'Your browser does not support the video tag.', 'youzify' ) . '</video>', $attachments, $attachment_id, $data );
					break;
				...
				case 'audio':
					...
					echo apply_filters( 'youzify_get_wall_post_audio', '<audio controls><source src="' . $audio_link . '" type="audio/mpeg">' . __( 'Your browser does not support the audio element.', 'youzify' ) . '</audio>', $audio_link, $attachment_id );
					break;
	...
	function get_wall_post_audio( $attachments ) {
		...
		foreach ( $attachments as $media_id => $data ) {
			...
			echo apply_filters( 'youzify_get_wall_post_audio', '<audio controls><source src="' . $audio_link . '" type="audio/mpeg">' . __( 'Your browser does not support the audio element.', 'youzify' ) . '</audio>', $audio_link );
		}
	...
	function get_wall_post_video( $attachments ) {
		...
		if ( ! empty( $attachments ) ) {
			...
			foreach ( $attachments as $media_id => $data ) {
				...
				echo apply_filters( 'youzify_get_wall_post_video', '<video width="100%" controls preload="metadata" type="video/quicktime" poster="' . $poster . '"><source src="' . wp_get_attachment_url( $media_id ) . '#t=0.001" type="video/mp4">' . __( 'Your browser does not support the video tag.', 'youzify' ) . '</video>', $media_id, $data );
			}
