...
class ERForms_Submission extends ERForms_Post {
    ...
    public function get_submissions_by_form($form_id, $args = array()) {
        $defaults = array(
            array(
                'key' => 'erform_form_id',
                'value' => $form_id,
        ...
        $meta_args = array('meta_query' => $defaults);
        $args = wp_parse_args($args, $meta_args );
        $posts = $this->get('', $args);
        $submissions = array();
    ...
    public function get_sub_count_by_form($form_id) {
        global $wpdb;
        $count = $wpdb->get_var( $wpdb->prepare("SELECT count(meta_id) as count FROM {$wpdb->prefix}postmeta WHERE meta_key = %s AND meta_value = %d",'erform_form_id',$form_id));
        return $count;
    }
