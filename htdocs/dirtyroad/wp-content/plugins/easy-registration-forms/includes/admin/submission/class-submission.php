...
class ERForms_Admin_Submission {
    ...
    */
    public function __construct() {
           ...
           add_filter('erf_admin_sub_bulk_action_html', array($this, 'sub_bulk_action_html'),10,2);
    }
   ...
   private function change_payment_status($sub_id){
    ...
    public function sub_bulk_action_html($html, $id){
        $html .= '<label for="bulk-action-selector-' . esc_attr( $id ) . '" class="screen-reader-text">' . __( 'Select Label', 'erforms' ) . '</label>';
        $html .= '<select style="display:none" name="action_label' . $id . '" id="bulk-action-label' . esc_attr( $id ) . "\">\n";
        $html .= '<option value="">' . __( 'Select Label' ) . "</option>\n";
        $labels= erforms()->label->get_labels();
        foreach($labels as $label){
            $html .= '<option value="'.esc_attr($label['id']).'">'.$label['name'].'</option>';
        }
        $html .="</select>\n";
        return $html;
    }
