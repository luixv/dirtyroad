jQuery( function( $ ) {
    ...
    if ( window.bpVerifiedMemberAdmin && window.bpVerifiedMemberAdmin.ajaxUrl ) {
        ...
        var initTooltips = function( tooltipContent ) {
            ...
            // Add tooltip to dom
            var $tooltip = $( '<span class="bp-verified-badge-tooltip" role="tooltip" style="visibility: hidden;">' + tooltipContent + '<span class="bp-verified-badge-tooltip-arrow" data-popper-arrow></span></span>' );
            $( this ).after( $tooltip );
        ...
        $verifiedBadges.each( function() {
            var tooltip = $( this ).parent().hasClass( 'bp-verified-by-role' ) ? window.bpVerifiedMemberAdmin.verifiedByRoleTooltip : window.bpVerifiedMemberAdmin.verifiedTooltip;
        ...
        $( 'a.bp-verified-member-toggle' ).on( 'click', function( e ) {
            ...
            // Bail if already loading or if the user belongs to a verified role
            if ( loading || $( this ).hasClass( 'bp-verified-by-role' ) )
                return;
