...
if ( ! defined( 'ABSPATH' ) ) { exit; }
 ...
 if ( ! defined( 'ABSPATH' ) ) { exit; } global $fs_active_plugins; $fs_options = FS_Options::instance( WP_FS__ACCOUNTS_OPTION_NAME, true ); $off_text = fs_text_x_inline( 'Off', 'as turned off' ); $on_text = fs_text_x_inline( 'On', 'as turned on' ); ?>
...
<?php foreach ( $module_types as $module_type ) : ?>
 ...
 if ( WP_FS__MODULE_TYPE_THEME !== $module_type ) { $is_active = is_plugin_active( $data->file ); } else { $current_theme = wp_get_theme(); $is_active = ( $current_theme->stylesheet === $data->file ); if ( ! $is_active && is_child_theme() ) { $parent_theme = $current_theme->parent(); $is_active = ( ( $parent_theme instanceof WP_Theme ) && $parent_theme->stylesheet === $data->file ); } } ?>
                ...
                <tr<?php if ( $is_active ) { if ( $fs->has_api_connectivity() && $fs->is_on() ) { echo ' style="background: #E6FFE6; font-weight: bold"'; } else { echo ' style="background: #ffd0d0; font-weight: bold"'; } } ?>>
                    <td><?php echo $data->id ?></td>
                    <td><?php echo $slug ?></td>
                    <td><?php echo $data->version ?></td>
                    <td><?php echo $data->title ?></td>
                    <td<?php if ( $is_active && ! $fs->has_api_connectivity() ) { echo ' style="color: red; text-transform: uppercase;"'; } ?>><?php if ( $is_active ) { echo esc_html( $fs->has_api_connectivity() ? fs_text_x_inline( 'Connected', 'as connection was successful' ) : fs_text_x_inline( 'Blocked', 'as connection blocked' ) ); } ?></td>
...
<?php foreach ( $module_types as $module_type ) : ?>
    <?php
 $sites_map = $VARS[ $module_type . '_sites' ]; $is_multisite = is_multisite(); $all_plans = false; ?>
    <?php if ( is_array( $sites_map ) && count( $sites_map ) > 0 ) : ?>
        ...
        <table id="fs_<?php echo $module_type ?>_installs" class="widefat">
            ...
            <?php foreach ( $sites_map as $slug => $sites ) : ?>
                <?php if ( ! is_array( $sites ) ) { $sites = array( $sites ); } ?>
...
<?php endforeach ?>
<?php
 $users = $VARS['users']; $users_with_developer_license_by_id = array(); foreach ( $module_types as $module_type ) { $licenses = $VARS[ $module_type . '_licenses' ]; foreach ( $licenses as $license ) { if ( $license->is_whitelabeled ) { $users_with_developer_license_by_id[ $license->user_id ] = true; } } } ?>
        ...
        <table id="fs_<?php echo $module_type ?>_licenses" class="widefat">
            ...
            <?php foreach ( $licenses as $license ) : ?>
                <tr>
                    ...
                    <td><?php
 echo $license->is_whitelabeled ? $license->get_html_escaped_masked_secret_key() : esc_html( $license->secret_key ); ?></td>
