...
class BPTK_Block
{
            ...
            case 'unblock':
                ...
                if ( wp_verify_nonce( $_REQUEST['token'], 'unblock-' . $_REQUEST['list'] ) ) {
                    ...
                    if ( ($key = array_search( $_REQUEST['num'], $current )) !== false ) {
                        $blocking_user = (int) $_REQUEST['list'];
                        unset( $current[$key] );
                        update_user_meta( $blocking_user, 'bptk_block', $current );
                        $blocked_by = get_user_meta( (int) $_REQUEST['num'], 'bptk_blocked_by', true );
                        ...
                        if ( $blocked_by ) {
                            ...
                            if ( $key !== false ) {
                                ...
                                update_user_meta( (int) $_REQUEST['num'], 'bptk_blocked_by', $blocked_by );
                            }
                        
                        }
                        
                        do_action( 'bptk_user_unblocked', (int) $_REQUEST['num'], $blocking_user );
                        bp_core_add_message( __( 'User successfully unblocked', 'bp-toolkit' ) );
                    } else {
                        echo  'Not in block list?' ;
                    }
            ...
            case 'block':
                ...
                if ( wp_verify_nonce( $_REQUEST['token'], 'block-' . $_REQUEST['list'] ) ) {
                    ...
                    if ( user_can( (int) $_REQUEST['num'], BPTK_ADMIN_CAP ) ) {
                        ...
                        if ( $blocked_by ) {
                            $key = array_search( (int) $_REQUEST['list'], $blocked_by );
                            
                            if ( $key === false ) {
                                $blocked_by[] = (int) $_REQUEST['list'];
                                update_user_meta( $blocked_user, 'bptk_blocked_by', $blocked_by );
                            }
                        ...
                        do_action( 'bptk_user_blocked', (int) $_REQUEST['num'], (int) $_REQUEST['list'] );
    ...
    public function remove_friendship( $blocker_id, $blocked_id )
    {
        ...
        error_log( 'Correct firing sequence' );
