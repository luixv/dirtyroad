...
class cbFieldHandler extends cbPluginHandler
{
	...
	protected function renderFieldHtml( $field, $user, $oValue, $output, $formatting, $reason, $rowClasses )
	{
		...
		if ( preg_match( '/^(?:<(select|input|textarea|button)|(?:<[^<]+(?:form|input|btn)-(group)[^>]+>))/i', trim( $oValue ), $matches ) ) {
	...
	protected function _fieldEditToHtml( &$field, &$user, $reason, $tag, $type, $value, $additional, $allValues = null, $displayFieldIcons = true, $classes = null, $translate = true )
	{
		...
		switch ( $type ) {
			case 'radio':
				...
				$htmlInput		=	moscomprofilerHTML::radioListTable( $allValues, $inputName, $additional, 'value', 'text', $value, $field->cols, $field->rows, $field->size, $oReq, null, $tooltip, $translate );
				break;
			case 'radiobuttons':
			...
			case 'multicheckbox':
				...
				$tooltip		=	( $trimmedDescription && $inputDescription ? cbTooltip( $_CB_framework->getUi(), $htmlDescription, $translatedTitle, null, null, null, null, 'data-hascbtooltip="true"' ) : null );
				$htmlInput		=	moscomprofilerHTML::checkboxListTable( $allValues, $inputName . '[]', $additional, 'value', 'text', $this->_explodeCBvaluesToObj( $value ), $field->cols, $field->rows, $field->size, $oReq, null, $tooltip, $translate );
				break;
			...
			case 'yesno':
				...
				$tooltip		=	( $trimmedDescription && $inputDescription ? cbTooltip( $_CB_framework->getUi(), $htmlDescription, $translatedTitle, null, null, null, null, 'data-hascbtooltip="true"' ) : '' );
				$htmlInput		=	moscomprofilerHTML::yesnoButtonList( $inputName, $tooltip, $value, null, null, $additional );
				break;
			...
			case 'predefined':
			case 'email':
				if ( in_array( $type, array( 'email', 'primaryemailaddress' ) ) ) {
					$type		=	'email';
