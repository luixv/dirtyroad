...
var cbHideFields = [];
var cbParamsSaveBefHide = [];
var cbSels = [];
var cbPreviousOnChangeValues = [];
	...
	for (var i=0;i<cbHideFields.length;i++) {
		...
		if ( inputDom === null ) {
			...
			for ( var j = 0; j < sels.length; j++ ) {
				if ( ! ( ( sels[j].tagName.toLowerCase() == 'input' ) || ( sels[j].tagName.toLowerCase() == 'select' ) || ( sels[j].tagName.toLowerCase() == 'textarea' ) ) ) {
					continue;
				}
